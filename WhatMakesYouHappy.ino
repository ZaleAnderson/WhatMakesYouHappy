...
// Default SSID name
const char* SSID_NAME = "What Makes You Happy?";

// Default main strings
#define SUBTITLE "A simple question asked by a curious human"
#define TITLE "So what is it anyway?"
#define BODY "Just type your answer in the box and hit submit :)"
#define POST_TITLE "Thank you for your answer!"
#define POST_BODY "What other people have said:"
#define PASS_TITLE "Responses:"
#define POSTSUB_TITLE "Your response has been recorded"
...
String footer() { 
  return "</div><div class=q><a>&#169;2022 Z.A. Studios</a></div>";
}

String header(String t) {
  String a = String(currentSSID);
  String CSS = "article { background: #E31C79; padding: 1.3em; }" 
    "body { color: #333; font-family: Century Gothic, sans-serif; font-size: 18px; line-height: 24px; margin: 0; padding: 0; }"
    "div { padding: 0.5em; }"
    "h1 { margin: 0.5em 0 0 0; padding: 0.5em; }"
    "input { width: 100%; padding: 9px 10px; margin: 8px 0; box-sizing: border-box; border-radius: 0; border: 1px solid #555555; border-radius: 10px; }"
    "label { color: #333; display: block; font-style: italic; font-weight: bold; }"
    "nav { background: #E31C79; color: #fff; display: block; font-size: 1.3em; padding: 1em; }"
...
String index() {
  return header(TITLE) + "<div>" + BODY + "</ol></div><div><form action=/post method=post><label>Your magical answer:</label>"+
    "<input type=string name=m></input><input type=submit value=Submit></form>" + footer();
}
...
String posted() {
  ...
  EEPROM.commit();
  return header(POSTSUB_TITLE) + POST_BODY + allPass + footer();
}

String pass() {
  return header(PASS_TITLE) + "<ol>" + allPass + "</ol><br><center><p><a style=\"color:black\" href=/>Back to Index</a></p><p><a style=\"color:black\" href=/clear>Clear responses</a></p></center>" + footer();
}

String ssid() {
  return header("Change SSID") + "<p>Here you can change the question. After pressing the button \"Change Question\" you will lose the connection, so reconnect to the new network.</p>" + "<form action=/postSSID method=post><label>New question:</label>"+
    "<input type=text name=s></input><input type=submit value=\"Change Question\"></form>" + footer();
}
...
String clear() {
  ...
  EEPROM.write(passEnd, '\0');
  EEPROM.commit();
  return header(CLEAR_TITLE) + "<div><p>List has been reset.</div></p><center><a style=\"color:black\" href=/pass>Back to responses</a></p></center>" + footer();
}
